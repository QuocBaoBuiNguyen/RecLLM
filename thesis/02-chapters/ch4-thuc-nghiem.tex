...
Amazon-Book cho phép đánh giá khả năng tổng quát của CoQLLM ngoài miền phim, nơi tín hiệu
cộng tác yếu và tỉ lệ người dùng/sản phẩm nguội cao.
...
\begin{itemize}
    ...
    \item \textbf{Kỹ thuật}: điều kiện hóa truy vấn theo người dùng bật trên cả hai tập dữ
    liệu.
...
trên $2{.}486$ người dùng tập kiểm tra.}
...
\hline
\textbf{CoQLLM (+ điều kiện hóa)} & \textbf{0{,}8147} & \textbf{0{,}5958} \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\textbf{Amazon-Book.} Đây là miền thưa và khó hơn. Với điều kiện hóa truy vấn theo người dùng,
CoQLLM đạt AUC $0{,}8147$ và uAUC $0{,}5958$ -- \textit{vượt cả CoLLM-MF và BinLLM (theo lần
chạy lại của CoRA) trên cả hai độ đo}. uAUC $0{,}5958$ vẫn thấp hơn CoRA-MF ($0{,}6262$), vốn
dùng cơ chế sinh trọng số khác hẳn; phân tích khác biệt AUC--uAUC (Mục~\ref{sec:nghich-ly})
cho thấy dư địa của uAUC trên miền này chủ yếu nằm ở chất lượng embedding MF cho nhóm người
dùng/sản phẩm nguội.
...
Trên MovieLens-1M, uAUC ở nhóm người dùng \textit{ấm} (warm, MF có tín hiệu) đạt $0{,}7254$,
cao hơn hẳn nhóm \textit{nguội} (cold) chỉ $0{,}5969$; trong khi AUC hai nhóm chênh ít hơn
($0{,}7787$ so với $0{,}7030$). Điều này cho thấy trần của uAUC không nằm ở cầu nối Q-Former
(nếu cầu nối yếu thì AUC cũng phải sập) mà ở \textit{chất lượng embedding MF cho người
dùng/sản phẩm nguội}. Đây là căn cứ để định hướng cải thiện uAUC về phía MF teacher
(Mục~\ref{sec:huong-phat-trien}) thay vì tinh chỉnh thêm kiến trúc cầu nối.

\section{Thí nghiệm loại bỏ thành phần}
\label{sec:ablation}

Bảng~\ref{tab:ablation} tách biệt đóng góp của kỹ thuật điều kiện hóa truy vấn theo người dùng
bằng cách bật/tắt cơ chế này trên nền cầu nối soft token (MovieLens-1M).

\begin{table}[ht]
\caption{Ablation kỹ thuật điều kiện hóa truy vấn theo người dùng trên MovieLens-1M, trên nền
soft token; chọn checkpoint theo uAUC.}
\label{tab:ablation}
\begin{center}
...
\textbf{Đọc bảng ablation.} Trên MovieLens-1M, điều kiện hóa truy vấn theo người dùng nâng AUC
từ $0{,}7335$ lên $0{,}7475$ ($+0{,}014$) trong khi uAUC gần như không đổi ($0{,}7009 \to
0{,}6968$). Kết quả này phù hợp với phân tích ở Mục~\ref{sec:nghich-ly}: phần dịch theo người
dùng chủ yếu nâng khả năng phân biệt \textit{giữa} các người dùng (AUC) và gần trung tính với
xếp hạng \textit{trong} từng người dùng (uAUC).
...
thuần; và (ii)~bật/tắt riêng biểu diễn đa token dưới cùng backbone Vicuna. Luận văn không điền
số ước lượng cho các hàng này.
...
(i)~véc-tơ cộng tác thô (đầu ra MF) và embedding token LLM tương ứng trước khi huấn luyện
cầu nối; (ii)~soft token đầu ra Q-Former và embedding token LLM sau khi huấn luyện.
...
\textit{ấm} (warm -- có embedding MF, tức MF đã ``thấy'' người dùng/sản phẩm trong huấn luyện)
và người dùng \textit{nguội} (cold -- MF không có embedding, phải dùng đại diện mặc định).
Bảng~\ref{tab:warm-cold} trình bày AUC/uAUC theo từng nhóm trên MovieLens-1M.

\begin{table}[ht]
\caption{Phân rã warm/cold trên tập kiểm tra MovieLens-1M (cấu hình đề xuất).}
\label{tab:warm-cold}
\begin{center}
...
Trên MovieLens-1M, nhóm warm đạt uAUC $0{,}7254$ -- cao hơn hẳn nhóm cold ($0{,}5969$) -- trong
khi AUC hai nhóm chênh ít hơn nhiều ($0{,}7787$ so với $0{,}7030$). Điều này khẳng định uAUC bị
chặn trên bởi \textit{chất lượng embedding MF cho người dùng/sản phẩm nguội} chứ không phải bởi
cầu nối: nếu cầu nối là nút thắt thì AUC ở nhóm cold cũng phải sập, nhưng AUC vẫn giữ ở mức
khá. Ở nhóm warm -- nơi MF cung cấp tín hiệu tốt -- CoQLLM khai thác hiệu quả hơn hẳn. Nhận định
này khớp với phân tích khác biệt AUC--uAUC (Mục~\ref{sec:nghich-ly}) và chỉ ra hướng cải thiện
tiếp theo nằm ở MF teacher trên nhóm cold (Mục~\ref{sec:huong-phat-trien}).
...
\begin{enumerate}
    ...
    \item \textbf{Các ablation thành phần chưa đầy đủ:} Việc bật/tắt riêng biểu diễn đa token
    chưa được chạy dưới cùng backbone Vicuna; kết luận về từng thành phần vì thế còn hạn chế.
    ...
    \item \textbf{Trần uAUC ở nhóm cold:} uAUC bị chặn trên bởi chất lượng embedding MF cho
