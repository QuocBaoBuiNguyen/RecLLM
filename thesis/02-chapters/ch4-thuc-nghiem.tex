...
chính là điều kiện để kiểm chứng cầu nối CF--LLM trên miền khó, nơi tín hiệu CF yếu và
...
\subsection{Phân chia dữ liệu và quy trình đánh giá}
...
\subsection{Các mô hình baseline}
\label{ssec:baseline}

\textbf{Baseline chính:}
\begin{itemize}
    \item \textbf{CoLLM}~\cite{collm2023}: tích hợp CF vào LLM qua MLP một lớp; cùng giao
    ...
    \item \textbf{BinLLM}~\cite{binllm2024}: mã hóa nhị phân CF vào LLM -- SOTA gần nhất
...
\begin{itemize}
    ...
    \item \textbf{MF}~\cite{bpr2009} và \textbf{DIN}: các mô hình CF thuần (không LLM),
    ...
    cáo AUC/uAUC theo quy trình CTR, nên không thể so sánh trực tiếp về số liệu; được
...
\begin{itemize}
    ...
    nền Llama-2), đông cứng hoàn toàn; khuôn mẫu chỉ dẫn USER/ASSISTANT.
    \item \textbf{MF}: phân rã ma trận BPR~\cite{bpr2009}, chiều ẩn $d = 256$, được huấn
    luyện và đông cứng trước khi tích hợp.
    ...
    \item \textbf{Đòn bẩy SOTA}: \texttt{user\_conditioned} bật trên cả hai tập dữ liệu;
    \texttt{align\_rank\_loss} ($\lambda = 0{,}2$, kèm \texttt{user\_grouped\_batch}) bật
    ...
    MF và LLM đông cứng. Không có tham số sinh trọng số. Tổng số tham số huấn luyện được rất
    nhỏ so với $7$ tỉ tham số đông cứng của LLM, phản ánh thiết kế tinh chỉnh hiệu quả tham số.
...
\textit{vanilla} (chỉ soft token) và \textit{+ user\_conditioned} (cấu hình đề xuất).
...
\hline
MF (chỉ CF)~\cite{bpr2009} & 0{,}6482 & 0{,}6361 \\
\hline
DIN (chỉ CF) & 0{,}7166 & 0{,}6459 \\
\hline
...
\hline
\textbf{CoQLLM (+ user\_conditioned)} & \textbf{0{,}7475} & \textbf{0{,}6968} \\
\hline
\end{tabular}
\end{center}
\end{table}
...
trên $2{.}486$ người dùng tập kiểm tra. Run-1 = + user\_conditioned; Run-2 = thêm
align\_rank\_loss.}
...
\hline
CoQLLM (+ user\_conditioned, Run-1) & 0{,}8147 & 0{,}5958 \\
\hline
\textbf{CoQLLM (+ align\_rank, Run-2)} & \textbf{0{,}8132} & \textbf{0{,}6062} \\
\hline
\end{tabular}
\end{center}
\end{table}

\textbf{MovieLens-1M.} Cấu hình đề xuất (+ user\_conditioned) đạt AUC $0{,}7475$ và uAUC
...
trong hai độ đo. So với cấu hình vanilla, \texttt{user\_conditioned} nâng AUC từ $0{,}7335$
...
\textbf{Amazon-Book.} Đây là miền thưa và khó hơn. Với \texttt{user\_conditioned} (Run-1),
CoQLLM đạt AUC $0{,}8147$ và uAUC $0{,}5958$ -- \textit{vượt cả CoLLM-MF và BinLLM (theo lần
chạy lại của CoRA) trên cả hai độ đo}. Bổ sung \texttt{align\_rank\_loss} (Run-2) nâng uAUC
...
\section{Phân tích nghịch lý AUC/uAUC}
\label{sec:nghich-ly}
...
tín hiệu CF nếu chỉ giúp phân biệt giữa các người dùng (thay đổi \textit{mức nền} điểm
...
mà ở \textit{chất lượng nhúng CF của MF cho người dùng/sản phẩm nguội}. Đây là căn cứ để
định hướng cải thiện uAUC về phía MF teacher (Mục~\ref{sec:huong-phat-trien}) thay vì tinh
chỉnh thêm kiến trúc cầu nối.

\section{Phân tích ablation}
\label{sec:ablation}

Bảng~\ref{tab:ablation} trình bày phân tích ablation các thành phần của CoQLLM.

Bảng~\ref{tab:ablation} tách biệt đóng góp của hai đòn bẩy huấn luyện bằng cách bật/tắt
từng cơ chế trên nền cầu nối soft token, trên cả hai tập dữ liệu.

\begin{table}[ht]
\caption{Ablation hai đòn bẩy huấn luyện. Trên MovieLens-1M: bật \texttt{user\_conditioned}.
Trên Amazon-Book: bật \texttt{user\_conditioned} (Run-1) rồi thêm \texttt{align\_rank\_loss}
...
\hline
MovieLens-1M & + user\_conditioned & \textbf{0{,}7475} & 0{,}6968 \\
\hline
Amazon-Book & + user\_conditioned (Run-1) & 0{,}8147 & 0{,}5958 \\
\hline
Amazon-Book & + align\_rank (Run-2) & 0{,}8132 & \textbf{0{,}6062} \\
\hline
\end{tabular}
\end{center}
\end{table}

\textbf{Đọc bảng ablation.} Hai đòn bẩy tác động lên hai độ đo khác nhau, đúng như thiết kế.
Trên MovieLens-1M, \texttt{user\_conditioned} là đòn bẩy \textit{AUC} (+$0{,}014$), gần trung
tính với uAUC. Trên Amazon-Book, \texttt{align\_rank\_loss} là đòn bẩy \textit{uAUC}
...
\texttt{align\_rank\_loss} không còn dư địa; trên Amazon-Book uAUC còn thấp ($\approx 0{,}59$)
...
thuần; (ii)~bật/tắt riêng biểu diễn đa token và tách projector dưới cùng backbone Vicuna;
(iii)~ablation \texttt{align\_rank\_loss} trên MovieLens-1M (kỳ
...
\subsection{Đo lường khoảng cách ngữ nghĩa}
\label{ssec:cosine-sim}

Để kiểm chứng rằng cầu nối Q-Former thực sự thu hẹp khoảng cách ngữ nghĩa giữa không
gian CF và không gian LLM, luận văn đo cosine similarity giữa:
(i)~véc-tơ CF thô (đầu ra MF) và véc-tơ nhúng token LLM tương ứng trước khi huấn luyện
...
chỉnh không gian CF về phía không gian ngữ nghĩa LLM.

\subsection{Phân tích theo nhóm người dùng (warm/cold)}
...
xấp xỉ mức ngẫu nhiên. Điều này khẳng định uAUC bị chặn trên bởi \textit{chất lượng nhúng CF
...
phân tích nghịch lý AUC/uAUC (Mục~\ref{sec:nghich-ly}) và chỉ ra đòn bẩy cải thiện tiếp theo
nằm ở MF teacher trên nhóm cold (Mục~\ref{sec:huong-phat-trien}).

\section{Thảo luận}
\label{sec:thao-luan}

\subsection{Đối chiếu với ILM về quy trình đánh giá}
\label{ssec:doi-chieu-ilm}

ILM~\cite{ilm2024} không báo cáo AUC/uAUC theo quy trình CTR, nên không thể so sánh
...
\subsection{Điểm mạnh và hạn chế của cầu nối Q-Former}
\label{ssec:diem-manh-han-che}

So với lớp chiếu tuyến tính của CoLLM, cầu nối Q-Former có cơ chế chọn lọc đặc trưng rõ
ràng hơn qua tập truy vấn học được và self-attention. Vì tín hiệu CF chỉ đi qua đường soft token, LLM
...
\begin{enumerate}
    ...
    tách projector, và \texttt{align\_rank\_loss} trên MovieLens-1M đều chưa được chạy
