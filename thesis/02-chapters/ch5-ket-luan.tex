...
hơn mô-đun chiếu MLP của CoLLM hay không? Để trả lời, luận văn thiết kế, hiện thực và đánh
...
CoQLLM thay thế mô-đun chiếu MLP pointwise của CoLLM bằng kiến trúc Q-Former, trong đó embedding
cộng tác của sản phẩm mục tiêu và của các sản phẩm trong lịch sử thay thế đặc trưng ảnh làm đầu
vào cross-attention, tín hiệu người dùng được đưa vào qua điều kiện hóa truy vấn, và toàn bộ
được chuyển thành soft token cho LLM. Trên nền đó, luận văn bổ sung kỹ thuật huấn luyện điều
kiện hóa truy vấn theo người dùng, cùng với biểu diễn đa token như một hệ quả của thiết kế
nhiều truy vấn. Kết quả chi tiết được trình bày ở Chương~\ref{Chapter4}.

\textbf{Đóng góp 2: Đánh giá có hệ thống theo độ đo AUC/uAUC trên hai tập dữ liệu.}
Luận văn đánh giá cầu nối Q-Former trên bài toán CTR với AUC và uAUC -- cùng quy trình với
CoLLM -- trên MovieLens-1M và Amazon-Book, cho phép so sánh trực tiếp với CoLLM, BinLLM và
CoRA. Phân tích ablation tách biệt đóng góp của từng kỹ thuật, và phân rã warm/cold làm rõ
trần uAUC nằm ở chất lượng embedding MF.
...
\begin{itemize}
    ...
    (+ điều kiện hóa) đạt AUC $0{,}7475$ và uAUC $0{,}6968$, vượt BinLLM
    ...
    điều kiện hóa truy vấn theo người dùng, CoQLLM đạt $0{,}8147$/$0{,}5958$, vượt CoLLM-MF và
    BinLLM (theo lần chạy lại của CoRA) trên cả hai độ đo; uAUC còn thấp hơn CoRA-MF
    ($0{,}6262$), phản ánh trần do chất lượng MF ở nhóm cold.

    \item \textbf{Điều kiện hóa truy vấn theo người dùng chủ yếu cải thiện AUC.} Cơ chế này
    nâng khả năng phân biệt \textit{giữa} các người dùng (AUC) và gần trung tính với xếp hạng
    \textit{trong} từng người dùng (uAUC), đúng như phân tích khác biệt AUC--uAUC.

    \item \textbf{Trần uAUC nằm ở MF teacher, không phải cầu nối.} Phân rã warm/cold trên
    MovieLens-1M cho thấy uAUC nhóm warm cao hơn hẳn nhóm cold ($0{,}7254$ vs $0{,}5969$), cho
    thấy hướng cải thiện tiếp theo nằm ở chất lượng embedding MF cho người dùng/sản phẩm nguội
    (cold).
\end{itemize}
...
\begin{enumerate}
    ...
    projector), đây là phần thực nghiệm còn để ngỏ.
...
\begin{itemize}
    ...
    nhóm người dùng/sản phẩm nguội -- điều mà phân rã warm/cold chỉ ra là trần thực sự
    của uAUC.
    \item Khảo sát các mục tiêu huấn luyện hướng tới thứ tự xếp hạng trong từng người dùng
    (chẳng hạn tổn thất BPR theo cặp) như một hướng cải thiện uAUC ở những miền còn nhiều dư địa
    xếp hạng.
\end{itemize}
